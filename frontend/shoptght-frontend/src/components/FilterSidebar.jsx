...
// ĐÃ THÊM PROP currentCategoryId ĐỂ ĐỒNG BỘ GIAO DIỆN RADIO BUTTON
const FilterSidebar = ({ currentCategoryId, onFilterChange }) => {
   ...
   return (
      <div className="w-full pr-4">
         ...
         {/* --- PHẦN 1: NHÓM SẢN PHẨM --- */}
         <FilterSection title="Nhóm sản phẩm">
            {[
               { name: 'Áo Thun', id: 1 },
               { name: 'Áo Polo', id: 2 },
               { name: 'Áo Sơ Mi', id: 3 },
               { name: 'Áo Tanktop', id: 4 },
               { name: 'Áo Khoác', id: 5 },
               { name: 'Quần Short', id: 6 },
               { name: 'Quần Jeans', id: 7 },
               { name: 'Quần Jogger', id: 8 },
               { name: 'Quần Lót', id: 9 },
               { name: 'Tất/Vớ & Phụ kiện', id: 10 },
            ].map((item) => (
               <label key={item.id} className="flex items-center gap-3 cursor-pointer group hover:bg-gray-50 p-1 rounded -ml-1">
                  <input
                     ...
                     checked={currentCategoryId === item.id} // <-- LOGIC ĐỒNG BỘ Ở ĐÂY
                     onChange={() => handleCategoryClick(item.id)}
                  />
                  <span className={`text-sm group-hover:text-blue-600 transition-colors font-medium ${currentCategoryId === item.id ? 'text-blue-600' : 'text-gray-600'}`}>
                     {item.name}
                  </span>
               </label>
            ))}
